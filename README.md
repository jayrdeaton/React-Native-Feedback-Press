...
By default, the first `useAudioPool` mounted anywhere in the app also configures the shared `expo-audio` session for `mixWithOthers` playback (`setAudioModeAsync({ interruptionMode: 'mixWithOthers', playsInSilentMode: true })`), once per app lifetime no matter how many pools mount after it. Without it, iOS leaves the session on its default `.soloAmbient` category, which stops whatever music or podcast the user already has playing and adds a session-negotiation delay before every feedback sound is actually heard. `useAudioPool` pays that cost once, up front, so short UI clips just play alongside whatever else is already going.

Pass `configureAudioSession: false` on a given pool to opt it out, for a host app that already manages `setAudioModeAsync` itself elsewhere - one that also does its own media playback or recording and wants a different session mode there. Because that configuration is a process-wide singleton rather than something scoped to one pool, the opt-out is process-wide too: once any pool mounts with `configureAudioSession: false`, no pool anywhere in the app - including ones that omit the option - will call `setAudioModeAsync` for the rest of the app's lifetime. It can only suppress configuration that hasn't happened yet, though, so it can't undo a call already made by a default (`true`) pool that mounted first; mount the opted-out pool as early as possible (e.g. alongside `FeedbackPressProvider`) if you're relying on it.
