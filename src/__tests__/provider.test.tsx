...
describe('FeedbackPressProvider', () => {
  ...
  it('applies initialValue prop as initial settings', () => {
    const wrapper = ({ children }: { children: React.ReactNode }) => <FeedbackPressProvider initialValue={{ vibrate: false }}>{children}</FeedbackPressProvider>
    const { result } = renderHook(() => useHapticSettings(), { wrapper })
  ...
  it('set updates settings in context', () => {
    ...
    const { result } = renderHook(() => useHapticSettings(), { wrapper })
    act(() => {
      result.current.set({ vibrate: false })
    })
    expect(result.current.settings.vibrate).toBe(false)
  })
  ...
  it('calls onChange when settings are updated via set', () => {
    ...
    const wrapper = ({ children }: { children: React.ReactNode }) => <FeedbackPressProvider onChange={onChange}>{children}</FeedbackPressProvider>
    const { result } = renderHook(() => useHapticSettings(), { wrapper })
    act(() => {
      result.current.set({ vibrate: false })
    })
